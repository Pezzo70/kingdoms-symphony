...
namespace Kingdom.Audio.Procedural
{
    ...
    {
        public static readonly float[] Fundamental = new float[]
        {
        ...
        public static KeyName RandomKey()
        {
            Array values = Enum.GetValues(typeof(KeyName));
            Random random = new Random();
            KeyName randomKey = (KeyName)values.GetValue(random.Next(values.Length));
            return randomKey;
        }

        public static KeyName[] ToNoteArray(this SimpleNotes simpleNote)
        {
            var keyEnum = Enum.GetNames(typeof(KeyName));
            var simpleNoteS = simpleNote.ToString();

            return keyEnum.Where(x => x.StartsWith(simpleNoteS)).Select(e => (KeyName)Enum.Parse(typeof(KeyName), e)).ToArray();
        }   
